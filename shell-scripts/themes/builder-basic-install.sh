#!/bin/zsh
source /Users/maxit/shared-zshrc/zshrc_general

###NOT COMPLETE
daa builder 0.4 theme "no" "yes" "stage"
cad espresso module dev "yes"

#daa newsletter 0.1 module "yes" "no" "stage"
#cad newsletter module test "no"

daa blog 0.1.12 module "yes" "no" "stage"
cad blog module test "no"

daa podcast 0.1 module "yes" "no" "stage"
cad podcast module test "no"

daa gallery 0.1 module "yes" "no" "stage"
cad gallery module test "no"

daa church 0.1 site "no" "no" "stage"

cs ckeditor test module
cs podcast test module